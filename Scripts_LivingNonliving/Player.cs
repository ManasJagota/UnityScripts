...
public class Player : ChooseOption
{
    ...
    {
        if(other.GetComponent<ObjectTagAssign>() != null)
        {
            if(other.GetComponent<ObjectTagAssign>().objTag == optionSelected)
            {
                score++;
                collectedText.text = score.ToString();
                audios.Play();
            ...
            else if (other.GetComponent<ObjectTagAssign>().objTag == wrongOption)
            {
                LevelFailed();
            }
